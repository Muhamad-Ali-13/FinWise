<x-app-layout>
                                ...
                                @foreach ($pemasukan as $key => $P)
                                    <tr class="border-b dark:border-gray-700 hover:bg-gray-600 dark:hover:bg-gray-700">
                                        <td class="px-4 py-2 sm:py-3">
                                            {{ $pemasukan->perPage() * ($pemasukan->currentPage() - 1) + $key + 1 }}
                                        </td>
                                        ...
                                        <td class="px-4 py-3 flex flex-col gap-2 sm:flex-row">
                                            ...
                                            <button type="button"
                                                ...
                                                onclick="return pemasukanDelete('{{ $P->id }}','{{ $P->kategori->nama_kategori }}')">
    ...
    <script>
        ...
        document.getElementById('addForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await axios.post('{{ route('pemasukan.store') }}', formData);
                Swal.fire({
                    position: 'top-end',
                    icon: 'success',
        ...
        document.getElementById('editForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await axios.post(`/pemasukan/${id}`, formData, {
                    headers: {
        ...
        function pemasukanDelete(id, nama_kategori) {
            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: `Anda akan menghapus pemasukan "${nama_kategori}".`,
                icon: 'warning',
