<x-app-layout>
    ...
    <script>
        const dataDelete = async (id, nama_kategori) => {
            let tanya = confirm(`Apakah anda yakin untuk menghapus pengeluaran kategori ${nama_kategori}?`);
            if (tanya) {
                try {
                    const response = await axios.post(`/pengeluaran/${id}`, {
                        '_method': 'DELETE',
                    ...
                    console.error(error);
                    alert('Terjadi kesalahan saat menghapus pengeluaran. Silakan cek konsol untuk detail.');
                }
            }
        };
    </script>
